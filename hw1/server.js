...
const server = http.createServer(function(req, res) {
  ...
  if (req.method == 'POST') {
    if (req.url == '/post') {
      res.write('This is a POST response');
      res.end();
    } else {
      res.writeHead(400);
      res.end('This is a wrong request');
    }

  }
  if (req.method == 'PUT') {
    if (req.url == '/put') {
      res.write('This is a PUT response');
      res.end();
    } else {
      res.writeHead(400);
      res.end('This is a wrong request');
  ...
  if (req.method == 'DELETE') {
    if (req.url == '/delete') {
      res.write('This is a DELETE response');
      res.end();
    } else {
      res.writeHead(400);
      res.end('This is a wrong request');
  ...
  if (req.method == 'PATCH') {
    if (req.url == '/patch') {
      res.write('This is a PATCH response');
      res.end();
    } else {
      res.writeHead(400);
      res.end('This is a wrong request');
